...
export function Nav({ theme, toggleTheme }: { theme: string; toggleTheme: () => void }) {
  ...
  return (
    <header className={`sticky top-0 z-50 mx-auto px-4 py-8 md:px-8 transition-all pointer-events-none ${scrolled ? "duration-1000 max-w-full md:max-w-[500px]" : "duration-1000 max-w-full md:max-w-3xl"}`}>
      <div className="relative">
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              className="md:hidden card absolute left-0 right-0 top-full mt-2 p-4 pointer-events-auto"
